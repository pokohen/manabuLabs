...
import type { Partner, PartnerLink, Banner } from '@/lib/schemas/partner'

type Tab = 'profile' | 'links' | 'banners'

export default function PartnerDashboard() {
  const { isPartner, partner } = usePartnerStore()
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-50 dark:bg-black p-4">
        <div className="text-center">
          <h1 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">파트너 전용 페이지</h1>
          <p className="text-zinc-600 dark:text-zinc-400 text-sm">
            이 페이지는 파트너만 이용할 수 있습니다.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black p-4">
      <div className="max-w-2xl mx-auto">
        ...
        {activeTab === 'links' && <LinksTab partnerId={partner!.id} />}
        {activeTab === 'banners' && <BannersTab partnerId={partner!.id} />}
      </div>
    </div>
  )
}
...
// 링크 관리 탭
function LinksTab({ partnerId }: { partnerId: string }) {
...
// 배너 관리 탭
function BannersTab({ partnerId }: { partnerId: string }) {
