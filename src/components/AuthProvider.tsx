...
export function AuthProvider() {
  ...
  // auth state 리스너 — DB 쿼리를 콜백 밖으로 분리
  useEffect(() => {
  ...
  // onAuthStateChange에서 user가 설정되면 DB 데이터를 별도로 로드
  const user = useAuthStore((s) => s.user)
  useEffect(() => {
    if (!user || !pendingUserId.current) return
  ...
  useEffect(() => {
    ...
    if (menu) {
      ...
      supabase
        ...
        .upsert(
          { user_id: u.id, last_studied_menu: menu.path, updated_at: new Date().toISOString() },
          { onConflict: 'user_id' }
        )
        .then()
    }
